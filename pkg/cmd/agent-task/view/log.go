...
func followLogs(fetcher func() ([]byte, error), w io.Writer, cs *iostreams.ColorScheme) error {
	var last string
	for {
		raw, err := fetcher()
		if err != nil {
		...
		if stop, err := printLogs([]byte(diff), w, cs); err != nil {
			return err
		} else if stop {
			return nil
		}
...
func printLogs(logs []byte, w io.Writer, cs *iostreams.ColorScheme) (bool, error) {
	...
	for _, line := range lines {
		...
		if stop, err := printLogEntry(entry, w, cs); err != nil {
...
func printLogEntry(entry logEntry, w io.Writer, cs *iostreams.ColorScheme) (bool, error) {
	var stop bool
